...
//        Element w_above_dot = Y1.pow(r_above_dot.toBigInteger().multiply(BigInteger.TWO)).mul(N.add(BigInteger.ONE).modPow(sigma_above_dot.mul(2).toBigInteger(), pairing.getZr().getOrder())).getImmutable();
...
//        Element v_above_dot = Y2.mul(Y3.pow(TransformUtils.H1FromBytesToL(u.toBytes(),w.toBytes()))).powZn(r_above_dot.mul(2)).getImmutable();
